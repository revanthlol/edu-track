...
const CourseForm = ({ course, departments, faculty, onSave, onCancel }) => {
    ...
    const [formData, setFormData] = useState({
        courseName: course?.courseName || '', description: course?.description || '', credits: course?.credits || '',
        facultyId: course?.facultyId?.toString() || 'none', departmentId: course?.departmentId?.toString() || 'none'
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            onSave();
        } catch (err) { alert('Operation failed. Please ensure the course name is unique.'); }
    };
    ...
    return (<form onSubmit={handleSubmit}><div className="grid gap-4 py-4">
        ...
        <div className="grid grid-cols-4 items-center gap-4"><Label className="text-right">Desc.</Label><Input value={formData.description} onChange={e=>setFormData({...formData, description: e.target.value})} className="col-span-3"/></div>
...
export default function AdminCourseManagementPage() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            setData({
                ...
                faculty: usersRes.data.users.filter(u => u.role === 'faculty'),
                departments: usersRes.data.departments
            });
        } catch(e) { console.error("Failed to load management data", e) }
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <Card><CardHeader><div className="flex justify-between items-center"><div><CardTitle>Course Management</CardTitle><CardDescription>Create, update, and manage all courses in the system.</CardDescription></div><Button onClick={()=>setDialogContent('create')}>Create New Course</Button></div></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Course</TableHead><TableHead>Faculty</TableHead><TableHead>Department</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
        ...
        <Dialog open={!!dialogContent} onOpenChange={(isOpen)=>!isOpen&&setDialogContent(null)}><DialogContent>
            <DialogHeader><DialogTitle>{dialogContent==='create'?'Create New Course':`Edit: ${dialogContent?.courseName}`}</DialogTitle><DialogDescription>Make changes to the course details here.</DialogDescription></DialogHeader>
            {dialogContent && <CourseForm course={dialogContent==='create'?null:dialogContent} departments={data.departments} faculty={data.faculty} onSave={handleSave} onCancel={()=>setDialogContent(null)}/>}</DialogContent></Dialog>
    </motion.div>);
}